...
const socialLinks = [
  ...
  { path: "#", icon: <AiFillInstagram className="group-hover:text-white w-4 h-5" /> },
        ...
        <div className="flex justify-between flex-col md:flex-row flex-wrap gap-[30px]">
          <div>
            ...
            <p className="text-[16px] leading-7 font-[400] text-textColor mt-4">
              Copyright {year} developed by Mr.Dev. All rights reserved.
            </p>
            ...
            <div className="flex items-center gap-3 mt-4">
              {socialLinks.map((link, index) => (
                <Link 
                  to={link.path} 
                  key={index}
                  className="w-9 h-9 border border-black rounded-full flex items-center justify-center group hover:bg-primaryColor hover:border-none"
                >
                  {link.icon}
                </Link>
              ))}
